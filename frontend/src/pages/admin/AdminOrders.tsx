          ...
          <table className="w-full min-w-[760px] border-collapse font-sans text-sm">
            ...
            <tbody>
              {orders.map((order) => {
                ...
                return (
                  <tr key={order.id} className="border-b border-ink-700 bg-ink-950 last:border-0">
                    <td className="px-4 py-3">
                      <Link to={`/admin/orders/${order.id}`} data-cursor-hover className="font-bold text-gold-400 hover:underline">
                        #{order.id}
                      </Link>
                    </td>
